...
class SolverTemplate : public QuadraticOptimizer {
  public:
    ...
    Eigen::VectorXd initial_guess(scenario_parameter* scenerio);
    Eigen::VectorXd lbx(scenario_parameter* scenerio);
    Eigen::VectorXd ubx(scenario_parameter* scenerio);
    Eigen::VectorXd lbg(scenario_parameter* scenerio);
    Eigen::VectorXd ubg(scenario_parameter* scenerio);
  ...
  private:
    ...
    int qp_return_status(int solver_status) {
        if (solver_status == OSQP_SOLVED  || solver_status == OSQP_SOLVED_INACCURATE || solver_status == OSQP_MAX_ITER_REACHED)
            return 0;
        else {
            std::cout << "osqp solver problem: " << solver_status << std::endl;
            return 1;
        }
    }
