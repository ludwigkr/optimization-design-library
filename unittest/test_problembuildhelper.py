...
"""    H[0] = (xopt[0]+xopt[0]);
    ...
    #         vector = casadi.SX.sym("vector", 3, 1)
    #         vector[0:2] = 0
    #         vector[2] = X**2
    #         ret = self.problem_build_helper.build_matrix("vector", vector, dense=True)
    #         target_output = \
    # """
    # """
    #         print(f"check: {ret}")
    #         self.assertTrue(True)
        ...
        with self.subTest("Matrix"):
            X = casadi.SX.sym("X", 1, 1)
            vector = casadi.SX.sym("vector", 2, 2)
            vector[0, 0] = 0
            vector[1, 0] = 0
            vector[0, 1] = 0
            vector[1, 1] = X**2
...
"""    double vector_temporary1 = 0;
    ...
    vector[0,1] = vector_temporary1;
    vector[1,0] = vector_temporary1;
    vector[1,1] = sq(X);"""
            self.assertTrue(ret == target_output)

            ret = self.problem_build_helper.build_matrix("vector", vector)
            target_output = \
"""    double vector_temporary1 = 0;
    vector[0] = vector_temporary1;
    vector[1] = vector_temporary1;
    vector[2] = vector_temporary1;
    vector[3] = sq(X);"""
            self.assertTrue(ret == target_output)

            ret = self.problem_build_helper.build_matrix("vector", vector, as_vector=False)
            target_output = \
"""    double vector_temporary1 = 0;
    vector(0,0) = vector_temporary1;
    vector(1,0) = vector_temporary1;
    ...
    vector(1,1) = sq(X);"""
            ...
            ret = self.problem_build_helper.build_matrix("vector", vector, as_vector=False, dense=True)
            target_output = \
"""    double vector_temporary1 = 0;
    vector[0,0] = vector_temporary1;
    vector[0,1] = vector_temporary1;
